...
class Lexi():
    ...
    def run_jarvis(self):
        ...
        while True:
            ...
            elif ('open teams'in self.command) or ('open stream'in self.command) or ('open sharepoint'in self.command) or('open outlook'in self.command)or('open sis login'in self.command)or('open octave'in self.command):
            ...
            elif ('open gmail'in self.command) or('open maps'in self.command) or('open calender'in self.command) or('open documents'in self.command )or('open spredsheet'in self.command) or('open images'in self.command) or('open drive'in self.command):
            ...
            #It can open : caliculator, notepad,paint, discord, spotify, ltspice,vscode(aka editor), steam, VLC media player
            elif ('open calculator'in self.command) or ('open notepad'in self.command) or ('open paint'in self.command) or ('open discord'in self.command) or ('open ltspice'in self.command) or ('open editor'in self.command) or ('open spotify'in self.command) or ('open steam'in self.command) or ('open media player'in self.command):
            ...
            #Command for find contact
            elif "find contact" in self.command:
                 query = self.command.replace("find contact", "").strip()
                 self.NameIntheContDataBase(query)
            ...
            #Individual => Eg: send a message to Athena
    ...
    def Intro(self):
        while True:                
            ...
            if ("wake up" in self.permission) or ("get up" in self.permission):
                self.run_jarvis()
            elif ("goodbye" in self.permission) or ("see you" in self.permission):
                self.talk("Thanks for using me boss, have a good day")
                sys.exit()
    ...
    def Fun(self,command):
        ...
        if 'your name' in command:
            ...
            self.talk("your name is Athena")
        elif 'university name' in command:
            self.talk("you are studing in kalasalingam university, with batcheloe in Computer Science and Artificail Intelligence") 
            ...
            self.talk("I am very young than you")
    ...
    def college(self,command):
        ...
        elif 'sis login' in command:
            self.talk('opening your sis login')
            webbrowser.open('https://sis.kalasalingam.ac.in/')
    ...
    def pdf_reader(self):
        ...
        self.talk("please enter the page number Which I need to read")
